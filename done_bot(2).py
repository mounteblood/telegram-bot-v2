  ...
  with open("log.txt", 'at') as logger:
    while True:
      ...
      for x in range(len(posts)):
        index_posts = range(len(posts))
        x = randint(0, len(index_posts))
        if x > len(posts):
          x -= 1
        if x >= limit.value:
          continue
        for dialog_id in dialog_list:
          if dialog_id < 0:
            try:
              ind = randint(0, len(d_accs[dialog_id]) - 1)
              client = d_accs[dialog_id][ind]
              try:
                while stop.value == 0:
                  time.sleep(0.1)
                if posts[x] == 0:
