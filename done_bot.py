                  ...
                  if no_post:
                    try:
                      with open(f"pic{x}.jpg", 'rb') as fphoto:
                        client.send_message(dialog.id, get_post_text(x), file = fphoto)
                        log_a_note(f"POST#{x}_CHANNEL {dialog.id} WITH PIC SUCCESSFUL")
                    except Exception as e:
                      client.send_message(dialog.id, get_post_text(x))
                      log_a_note(f"POST#{x}_CHANNEL {dialog.id} WITHOUT PIC SUCCESSFUL")
                      print("sended")
            except Exception as e:
              pass
        if no_post:
